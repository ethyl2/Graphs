...
1. To create 100 users with an average of 10 friends each, how many times would you need to call `add_friendship()`? 500 times. Why? 100 users \* 10 friends each // 2 = 500. Calling add_friendship() adds 2 connections because our social network is set up to have all bidirectional edges.

2. If you create 1000 users with an average of 5 random friends each, what percentage of other users will be in a particular user's extended social network? My implementation can't handle 1000 users yet, but 150 users with ave 5 friends yields almost 100%. What is the average degree of separation between a user and those in his/her extended network? Again, my implementation isn't ready to generate a network with 1000 users, but 150 users with ave 5 friends yields around an average 3 degrees of separation.
